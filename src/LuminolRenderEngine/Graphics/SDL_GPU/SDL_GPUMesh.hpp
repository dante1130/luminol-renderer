...
// time - callers never need a separate per-submesh base). bounds_center/
// bounds_radius and local_bounds_min/max are both local-space (transformed
// by each instance's model matrix at cull time) and describe the SAME
// cluster - the sphere is kept for the frustum test (cheap, and frustum
// culling isn't shape-sensitive), while local_bounds_min/max exists because
// a bounding sphere is a poor fit for thin/elongated meshlets (a curtain
// fold, a column edge, foliage): its radius can be much larger than the
// cluster's true cross-section, so occlusion sample points placed on the
// sphere's surface can land in genuinely-occluded space next to a real,
// thinner, visible surface - the same reason SDL_GPUMesh::local_bounds (see
// compute_mesh_local_bounds) uses an AABB instead of a sphere for Phase A's
// per-submesh test. local_bounds_min/max carry a trailing 0.0F pad each to
// keep them float4-sized (see SDL_GPUInstanceCullPass.cpp's
// SubmeshCullMetadata for why raw float3 array fields aren't used directly
// in a StructuredBuffer element). 64 bytes total, already a multiple of the
// 16-byte StructuredBuffer element stride - no extra padding field needed.
struct GpuMeshletMetadata {
    ...
    float bounds_radius;
    std::array<float, 4> local_bounds_min;
    std::array<float, 4> local_bounds_max;
};
