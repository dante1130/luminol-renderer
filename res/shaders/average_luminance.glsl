...
void main() {
    ...
    const float luminance_last_frame =
        imageLoad(average_luminance_image, ivec2(0, 0)).r;
    ...
    const float adapted_luminance =
        ...
        (weighted_average_luminance - luminance_last_frame) * time_coefficient;

    imageStore(
        average_luminance_image,
        ivec2(0, 0),
        vec4(adapted_luminance, 0.0, 0.0, 0.0)
    );
}
