...
layout(std430, binding = 2) buffer LuminanceHistogramBuffer {
    uint luminance_histogram[histogram_bins];
};

uint color_to_bin(
    vec3 color, float min_log_luminance, float inverse_log_luminance_range
) {
    ...
    const float normalized_log_luminance = clamp(
...
void main() {
    ...
    const ivec2 image_size = imageSize(hdr_image);

    if (texel.x >= image_size.x || texel.y >= image_size.y) {
        return;
    }
    ...
    const uint bin =
        color_to_bin(color, min_log_luminance, inverse_log_luminance_range);
