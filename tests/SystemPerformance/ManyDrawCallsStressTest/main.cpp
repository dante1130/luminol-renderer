...
// Headless stress test exercising many individual (non-instanced) queue_draw
// calls per frame, one per grid position, instead of a single batched
// instanced draw. This stresses per-draw-call CPU overhead in Renderer /
// SDL_GPUMeshRenderPass, a code path neither InstancedCubeStressTest nor
// DynamicInstancesStressTest touches (both issue one draw call for the whole
// grid via instancing).
//
...
auto main() -> int {
    ...
    auto luminol_engine = RenderEngine(Properties{
        .title = "Luminol Many Draw Calls Stress Test",
    });
    ...
    std::printf(
        "ManyDrawCalls stress test: %d draw calls, %d frames measured "
    ...
    if (!success) {
        std::printf(
            "ManyDrawCalls stress test FAILED: average %.3f ms/frame "
            "exceeds threshold %.3f ms/frame\n",
            average_frame_time_ms,
            max_average_frame_time_ms
        );
    } else {
        std::printf("ManyDrawCalls stress test PASSED\n");
    }
