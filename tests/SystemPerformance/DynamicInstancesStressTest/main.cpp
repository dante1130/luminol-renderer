...
// Headless stress test mirroring InstancedCubeStressTest's grid-of-cubes
// scene, but driving queue_draw_instanced (re-submitted every frame) instead
// of queue_draw_instanced_static (uploaded once). This isolates the cost of
// the per-frame CPU-copy + GPU-re-upload instancing path - the sibling of
// the static path that a prior commit optimized - which the static-only
// InstancedCubeStressTest never exercises.
//
...
constexpr auto max_average_frame_time_ms = 14.0;
...
auto main() -> int {
    ...
    auto luminol_engine = RenderEngine(Properties{
        .title = "Luminol Dynamic Instances Stress Test",
    });
    ...
    std::printf(
        "DynamicInstances stress test: %d instances, %d frames measured "
    ...
    if (!success) {
        std::printf(
            "DynamicInstances stress test FAILED: average %.3f ms/frame "
            "exceeds threshold %.3f ms/frame\n",
            average_frame_time_ms,
            max_average_frame_time_ms
        );
    } else {
        std::printf("DynamicInstances stress test PASSED\n");
    }
